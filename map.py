...
def print_info(msg: str) -> None:
    ...
    """
    prtstr = str(int(round(float(perf_counter() - START), 4) * 10000)) + " INFO: " + msg
    print(prtstr)
...
def print_errs(msg: str) -> None:
    """
        ...
        @param msg The message need to print.
    """
    prtstr = str(int(round(float(perf_counter() - START), 4) * 10000)) + " ERRS: " + msg
    print(prtstr)
...
if __name__ == '__main__':
    try:
        ...
        if ver < (3, 6, 0):
            print_errs("Your python version is too old. Please update to Python 3.6.0")
            sys.exit()
        ...
        while True:
            ...
            if not cities_view:
                frame = paint_chinese_opencv(frame, "中国疫情\n  地图", (470, 10), (0, 0, 0), 30)
                for level in range(1, 8, 1):
                    ...
                    img_draw.rectangle(((0, level * 20 - 20), (8, level * 20)), fill=color)
                ...
                if showmode == 0:
                    ...
                    cv2.rectangle(frame, (155, 348), (155, 348), (0, 0, 0), 50)
                    img_pil = Image.fromarray(frame)
                    img_draw = ImageDraw.Draw(img_pil)
                    img_draw.rectangle(((95, 323), (125, 373)), fill=(255, 0, 0))
                    img_draw.rectangle(((125, 323), (155, 373)), fill=(0, 0, 0))
                    frame = numpy.array(img_pil)
                    frame = paint_chinese_opencv(frame, "现有", (80, 338), (255, 255, 255), 20)
                    frame = paint_chinese_opencv(frame, "累计", (130, 338), (255, 255, 255), 20)
                else:
                    cv2.rectangle(frame, (95, 348), (95, 348), (0, 0, 0), 50)
                    cv2.rectangle(frame, (155, 348), (155, 348), (255, 0, 0), 50)
                    img_pil = Image.fromarray(frame)
                    img_draw = ImageDraw.Draw(img_pil)
                    img_draw.rectangle(((95, 323), (125, 373)), fill=(0, 0, 0))
                    img_draw.rectangle(((125, 323), (155, 373)), fill=(255, 0, 0))
                    frame = numpy.array(img_pil)
                    frame = paint_chinese_opencv(frame, "现有", (80, 338), (255, 255, 255), 20)
                    frame = paint_chinese_opencv(frame, "累计", (130, 338), (255, 255, 255), 20)
                if not select_asked:
                    ...
                    frame = paint_chinese_opencv(frame, confirm, (477, 85), (139, 0, 0), 12)
                    frame = paint_chinese_opencv(frame, nowconfirm, (537, 85), (240, 0, 240), 12)
                    frame = paint_chinese_opencv(frame, local, (477, 145), (139, 69, 19), 12)
                    frame = paint_chinese_opencv(frame, imported, (537, 145), (0, 0, 139), 12)
                    frame = paint_chinese_opencv(frame, no_infect, (477, 205), (125, 125, 125), 12)
                    frame = paint_chinese_opencv(frame, severe, (537, 205), (139, 0, 0), 12)
                    frame = paint_chinese_opencv(frame, suspect, (477, 265), (238, 197, 145), 12)
                    frame = paint_chinese_opencv(frame, heal, (537, 265), (0, 139, 0), 12)
                    frame = paint_chinese_opencv(frame, dead, (507, 325), (125, 125, 125), 12)
                    cv2.rectangle(frame, (475, 80), (525, 130), (0, 0, 139), 2)
                    cv2.rectangle(frame, (535, 80), (585, 130), (240, 0, 240), 2)
                    cv2.rectangle(frame, (475, 140), (525, 190), (19, 69, 139), 2)
                    cv2.rectangle(frame, (535, 140), (585, 190), (139, 0, 0), 2)
                    cv2.rectangle(frame, (475, 200), (525, 250), (125, 125, 125), 2)
                    cv2.rectangle(frame, (535, 200), (585, 250), (0, 0, 139), 2)
                    cv2.rectangle(frame, (475, 260), (525, 310), (145, 197, 238), 2)
                    cv2.rectangle(frame, (535, 260), (585, 310), (0, 139, 0), 2)
                    cv2.rectangle(frame, (505, 320), (555, 370), (125, 125, 125), 2)
                else:
                    area = select
                    add = ADD_SORT[AREAS_SORT.index(area)]
                    now = NOW_SORT[AREAS_SORT.index(area)]
                    confirm = CONFIRM_SORT[AREAS_SORT.index(area)]
                    cv2.rectangle(frame, (475, 150), (525, 200), (240, 0, 240), 2)
                    cv2.rectangle(frame, (535, 150), (585, 200), (0, 0, 139), 2)
                    cv2.rectangle(frame, (505, 210), (555, 260), (19, 69, 139), 2)
                    nowstr = f"现有病例\n{now}"
                    confirmstr = f"累计确诊\n{confirm}\n{number2str_with_sum_or_sub(add)}"
                    localstr = f"本土确诊\n{LOCAL_SORT[AREAS_SORT.index(area)]}"
                    left = (100 - len(area) * 25) / 2
                    frame = paint_chinese_opencv(frame, area, (475 + left, 90), (0, 0, 139), 30)
                    cv2.rectangle(frame, (480, 290), (580, 330), (0, 0, 0), 2)
                    frame = paint_chinese_opencv(frame, nowstr, (477, 155), (240, 0, 240), 12)
                    frame = paint_chinese_opencv(frame, confirmstr, (537, 155), (139, 0, 0), 12)
                    frame = paint_chinese_opencv(frame, localstr, (507, 215), (139, 69, 19), 12)
                    frame = paint_chinese_opencv(frame, " 详细信息", (480, 300), (0, 0, 0), 20)
                    ...
                    if isupdate:
                        ...
                        update_text = "未更新"
                    frame = paint_chinese_opencv(frame, update_text, (500, 350), update_color, 20)
                ...
                select_lst = [CONFIRM_SORT[AREAS_SORT.index(select)], NOW_SORT[AREAS_SORT.index(select)],
                              ADD_SORT[AREAS_SORT.index(select)], DEAD_SORT[AREAS_SORT.index(select)],
                ...
                frame = paint_chinese_opencv(frame, "绿色为已更新", (525, 0), (0, 139, 0), 12)
                frame = paint_chinese_opencv(frame, "红色为未更新", (525, 20), (139, 0, 0), 12)
                for pos in positions:
                    ...
                    cv2.rectangle(frame, newpos, newpos, (0, 0, 0), int(final * 2))
                ...
                for index, city in enumerate(citylst):
                    ...
                    frame = paint_chinese_opencv(frame, str(city[list(city.keys())[0]][4]), (558, y), (0, 0, 0), 12)
        ...
        prtstr = str(int(round(float(perf_counter() - START), 4) * 10000)) + " ERRS: " + str(e)
        print(prtstr)
        sys.exit()
else:
    del START
